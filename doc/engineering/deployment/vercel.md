...
1.  회원가입 성공 후 /friends 로 이동하는지 확인한다.
...
4.  저장된 출발 위치 조회, 저장, 사용 시각 갱신 API가 정상 동작하는지 확인한다.
5.  추천 API가 midpoint, summary, places 구조와 summary.mode, summary.category, summary.radiusUsed 값을 포함해 반환하는지 확인한다.

...
MeetPoint에서 Supabase는 사용자, 친구, 메시지, 현재 위치 공유값, 저장된 출발 위치 데이터를 저장하는 데이터베이스 역할을 맡는다.
...
3.  .env.local에 넣는 NEXT\_PUBLIC\_SUPABASE\_URL, NEXT\_PUBLIC\_SUPABASE\_PUBLISHABLE\_KEY, SUPABASE\_SERVICE\_ROLE\_KEY와는 다른 값이다.
...
Supabase 화면과 공식 자료 기준으로, MeetPoint 문서와 코드의 환경 변수명은 publishable key 기준으로 통일한다.
...
현재 프로젝트 세팅은 NEXT\_PUBLIC\_SUPABASE\_PUBLISHABLE\_KEY 기준으로 맞추면 된다.
...
이 프로젝트의 MVP 기준 실제 생성 대상 테이블은 아래 4개다.
...
### 9.2 users, friends, messages, departure\_locations 테이블 생성 SQL
...
4.  departure\_locations: 저장된 출발 위치와 최근 사용 위치 저장
...
1.  SQL Editor 열기
2.  테이블 생성 SQL 실행
3.  Table Editor에서 users, friends, messages, departure\_locations 생성 여부 확인
4.  API URL과 키 다시 확인
5.  로컬 .env.local 업데이트
...
4.  에러가 없는지 확인
5.  users, friends, messages, departure\_locations 생성 SQL 전체를 붙여 넣기
6.  Run 실행
7.  성공 메시지 확인
8.  Table Editor로 이동해서 users, friends, messages, departure\_locations 테이블이 실제로 생겼는지 확인
...
users, friends, messages, departure\_locations 생성 SQL과 인덱스 SQL 전체를 한 번에 실행한다.
...
4.  Table Editor에 departure\_locations 테이블이 보이는가
5.  users에 nickname\_normalized unique 제약이 들어갔는가
6.  friends에 user\_id, friend\_id 외래 키가 들어갔는가
7.  messages에 sender\_id, receiver\_id 외래 키가 들어갔는가
8.  departure\_locations에 location\_kind check, last\_used\_at 기본값이 들어갔는가
...
4.  테이블 생성 순서가 users -> friends -> messages -> departure\_locations 순서인지 확인

특히 friends, messages, departure\_locations 는 users를 참조하므로, users가 먼저 만들어져 있어야 한다.
...
### 14.3 브라우저에서 바로 users, friends, messages, departure\_locations 테이블에 쓰는 경우
...
6.  SQL Editor에서 users, friends, messages, departure\_locations 생성
7.  Next.js 서버 전용 Supabase client 유틸 작성
8.  repository 함수 작성
9.  Route Handler에서 repository 연결
10.  회원가입, 친구, 메시지, 저장된 출발 위치 API 순서대로 검증
...
7.  SQL Editor에서 users, friends, messages, departure\_locations를 만들었는가

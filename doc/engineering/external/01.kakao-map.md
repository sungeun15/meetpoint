...
![JavaScript 키 복사 화면](../../image/3.JavaScript%20SDK%20%ED%82%A4%EB%B3%B5%EC%82%AC.png)
...
![JavaScript SDK 도메인 설정 화면](../../image/2.JavaScript%20SDK%20%EB%8F%84%EB%A9%94%EC%9D%B8%EC%84%A4%EC%A0%95.png)
...
2.  `app/components/chat/recommendation/map/kakao-map-preview.tsx`: 추천 지도 프리뷰와 마커 렌더링 진입점
3.  `app/components/chat/recommendation/map/chat-recommendation-map-panel.tsx`: 추천 지도 패널 조합
4.  `app/components/chat/recommendation/map/mobile/chat-recommendation-map-mobile-layout.tsx`: 모바일 전체화면 지도 레이아웃
5.  `app/components/chat/recommendation/map/fullscreen/chat-recommendation-map-fullscreen-overlay.tsx`: 데스크톱 전체화면 지도 오버레이
6.  `lib/kakao/local.ts`: 서버에서 Kakao Local API 호출
7.  `app/api/recommendations/route.ts`: 추천 API에서 서버 호출 연결
...
4.  recommendationResult, 선택 장소, 모바일 시트 상태가 바뀌면 지도 상태를 함께 갱신
...
```
// app/components/chat/map-card.tsx
"use client";

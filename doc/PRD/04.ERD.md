...
기술 설계 문서에서 정의한 users, friends, messages 중심 구조를 데이터 관점에서 다시 정리하고, 팀원 간 테이블 관계 해석을 통일하는 것을 목표로 한다.

---

## 2\. ERD 범위

MVP 1차 범위의 기본 데이터 구조는 다음 3개 테이블을 중심으로 구성한다.
...
*   users 는 닉네임 기반 사용자 진입과 마지막 위치 정보를 저장한다.
...
```mermaid
erDiagram
    %% USERS 는 닉네임 기반 사용자 식별과 마지막 위치 저장을 담당한다.
    USERS {
        uuid id PK
        string nickname
...
*   users 조회, friends 조회, messages 조회는 모두 Next.js Route Handler 를 통해 수행한다.

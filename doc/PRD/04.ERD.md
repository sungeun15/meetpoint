...
이 문서는 MeetPoint의 후속 데이터 저장 구조를 시각적으로 고정하기 위한 ERD 문서이다.

현재 Team A 소스는 friends, chat, 저장 위치, 추천 결과를 로컬 상태와 fixture 데이터로 우선 표현하고 있으므로, 본 문서는 이후 DB 연동이 시작될 때 사용할 users, friends, messages, departure\_locations 중심 구조를 데이터 관점에서 다시 정리하는 것을 목표로 한다.

---

## 2\. ERD 범위

현재 UI-only 소스에는 실제 DB 테이블이 아직 연결되어 있지 않다. 아래 ERD는 후속 MVP DB 연동 단계에서 기본 데이터 구조로 사용할 4개 테이블을 중심으로 구성한다.
...
추가로 recommendations 테이블은 후속 확장용 구조로만 정의하며, DB 1차 연동 범위에서는 실제 생성 대상에 포함하지 않는다.

### 2.1 현재 구현 대응 관계

현재 Team A 소스와 ERD의 대응 관계는 다음과 같이 이해한다.

*   friends 테이블은 현재 friends-screen 과 friends-sidebar 에서 사용하는 로컬 친구 목록 상태의 후속 저장 구조이다.
*   messages 테이블은 현재 use-chat-screen-state 와 chat-conversation-panel 에서 사용하는 로컬 메시지 배열의 후속 저장 구조이다.
*   departure\_locations 테이블은 현재 use-recommendation-flow-state 의 저장 위치 목록 상태의 후속 저장 구조이다.
*   users 의 마지막 위치 컬럼은 현재 위치 상태 패널과 위치 공유 UI가 후속으로 저장될 대상 구조이다.
*   recommendations 는 현재 추천 카드 UI를 즉시 저장하지 않으므로 확장용 구조로만 유지한다.

---

## 3\. 엔터티 관계 요약

*   후속 users 는 닉네임 기반 계정 정보와 마지막 위치 정보를 저장한다.
*   후속 friends 는 사용자 간 친구 관계를 저장한다.
*   후속 messages 는 두 사용자 간 댓글형 채팅 메시지를 저장한다.
*   후속 departure\_locations 는 나중에 만나기 모드에서 재사용할 저장된 출발 위치를 저장한다.
*   recommendations 는 추천 결과 저장 확장용 테이블이며, 현재 UI 단계에서는 선택 사항이다.
...
*   이 다이어그램은 현재 UI 런타임 상태가 아니라 후속 DB 연동 시점의 저장 구조를 나타낸다.

---

## 5\. 테이블 상세 설명

아래 테이블 설명은 모두 후속 DB 연동 기준으로 이해한다. 현재 Team A 소스는 같은 정보를 로컬 상태와 목업 데이터로 우선 표현한다.
...
후속 MVP DB 연동 단계에서 실제 생성 대상은 다음과 같다.
...
후속 MVP DB 연동 단계에서 생성하지 않는 대상은 다음과 같다.

*   recommendations

이 기준은 현재 기술 설계 문서의 후속 Supabase 및 Route Handler 설계와 동일하게 유지한다.
...
*   현재 Team A 소스에서는 회원가입, 로그인, users 조회, friends 조회, messages 조회를 실제 DB 대신 로컬 상태와 fixture 데이터로 표현한다.
*   후속 연동 단계에서는 회원가입, 로그인, users 조회, friends 조회, messages 조회를 모두 Next.js Route Handler 를 통해 수행한다.
*   후속 연동 단계에서도 브라우저는 Supabase 직접 쓰기를 하지 않는다.
*   nickname 비교와 중복 검사는 nickname\_normalized 기준으로 처리한다.
*   후속 인증은 JWT를 httpOnly 쿠키에 저장하는 방식으로 처리하며, 닉네임 찾기와 비밀번호 재설정은 MVP에서 제외한다.
...
*   현재 추천 결과 카드는 저장하지 않고 즉시 표시만 하므로 recommendations 는 확장용 구조로만 유지한다.